...
namespace BCIEssentials.Networking
{
    public class LSLResponseStream : MonoBehaviour
    {
        ...
        public string CloseResponseStream()
        {
            // Try to close the stream
            try
            {
                responseInlet.close_stream();
                Debug.Log("Closing the response stream....");
            }
            catch (Exception e)
            {
                Debug.Log(e.Message);
                return "There was an error in closing the response stream: " + e.Message;
            }

            return "Response Stream (inlet to Unity) now closed";
        }
